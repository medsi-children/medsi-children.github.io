(function(){
  ...
  if(!transport.__medsiForcedParentWrapped){
    ...
    transport.chats=async function(session,bucket){
      ...
      if(forced&&res&&Array.isArray(res.chats)&&!res.chats.some(x=>phone10(x&&x.phone)===phone10(forced.phone))){
        res.chats.push({phone:phone10(forced.phone),parentName:forced.parentName||'',childName:forced.childName||'',lastSide:'',lastType:'text',lastText:'Нет сообщений',hasUnread:false,pinnedBucket:''});
      }
      return res;
    };
  }
  ...
  api.mount=function(overlay,state){
    ...
    function showNew(){document.body.dataset.screen='screenNewChat';title.textContent='Написать родителю';meta.textContent='Выберите родителя.';screenChats.classList.add('hidden');screenThread.classList.add('hidden');screen.classList.remove('hidden');requestAnimationFrame(()=>screen.classList.add('overlay-screen-ready'))}
    function showChats(){document.body.dataset.screen='screenChats';title.textContent='Чат с родителями';meta.textContent='Выберите чат с нужным родителем.';screen.classList.add('hidden');screen.classList.remove('overlay-screen-ready');screenThread.classList.add('hidden');screenChats.classList.remove('hidden');requestAnimationFrame(()=>screenChats.classList.add('overlay-screen-ready'))}
    function render(rows){
      list.replaceChildren();rows=Array.isArray(rows)?rows:[];renderedSignature=rowsSignature(rows);
      ...
      rows.forEach(r=>{
        ...
        card.append(name,del,info);card.addEventListener('click',e=>{if(e.target.closest('.chat-delete-toggle'))return;bridgeToCard(r,'open')});list.appendChild(card);
      });
    }
    ...
    function bridgeToCard(parent,mode){
      clearTimeout(bridgeTimer);window.__medsiForcedEducatorParent=parent;root.classList.add('educator-new-chat-bridging');btnRefresh.click();const started=Date.now();
      const probe=()=>{
        if(disposed)return;const card=findChatCard(parent.phone);
        if(card){window.__medsiForcedEducatorParent=null;if(mode==='delete'){const control=card.querySelector('.chat-delete-toggle');if(control)control.click();showNew();root.classList.remove('educator-new-chat-bridging');if(deleteModal){const observer=new MutationObserver(()=>{if(deleteModal.classList.contains('hidden')){observer.disconnect();setTimeout(()=>{if(!disposed){rowsCache=null;renderedSignature='';showNew();loadParents()}},120)}});observer.observe(deleteModal,{attributes:true,attributeFilter:['class']})}}else{screen.classList.add('hidden');card.click();root.classList.remove('educator-new-chat-bridging')}return}
        if(Date.now()-started>4500){window.__medsiForcedEducatorParent=null;root.classList.remove('educator-new-chat-bridging');showNew();error.textContent='Не удалось открыть чат родителя.';error.classList.remove('hidden');return}
