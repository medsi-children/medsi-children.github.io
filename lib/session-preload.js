...
async function callAppsScript(payload) {
  ...
  // Read-only requests may safely be repeated after a transient upstream
  // failure. Writes stay single-shot because the Apps Script operation may
  // have reached the table even when its response was lost.
  const retryable = new Set([
    'getD1ChatSession', 'verifyTutorSession', 'verifyTutorAccess',
    'getParentBootstrap', 'getParentRegistrationStatus',
    'getParentReauthorizationStatus', 'getReportSubmissionStatus',
    'listAvailableParentsForChat', 'listPendingParentReauthorizations'
  ]);
  let response;
  let lastError;
  for (let attempt = 0; attempt < (retryable.has(method) ? 2 : 1); attempt++) {
    try {
      ...
      lastError = error;
      if (attempt === 0 && retryable.has(method)) await new Promise(resolve => setTimeout(resolve, 350));
    }
  }
  if (!response) throw lastError || new Error('APPS_SCRIPT_UNAVAILABLE');
    ...
    if (req.method === 'POST' && req.url === '/__session/apps-script') {
      ...
      try {
        ...
        const allowed = new Set([
          'getD1ChatSession', 'verifyTutorSession', 'verifyTutorAccess',
          'listAvailableParentsForChat', 'listPendingParentReauthorizations',
          'getParentBootstrap', 'getParentRegistrationStatus', 'registerParent',
          'requestParentReauthorization', 'getParentReauthorizationStatus',
          'markEducatorMessagesAsRead', 'markReportAsRead',
          'decideParentReauthorization', 'appendReport',
          'getReportSubmissionStatus', 'deleteReportChildByPhone'
        ]);
