...
function extractSession(value) {
  ...
  if (value.d1Session && value.d1Session.token) return value.d1Session;
  if (value.session && value.session.token) return value.session;
  if (value.token && value.expiresAt) return value;
  return null;
}

function replaceSession(result, method, args) {
  if (!result || !key) return result;
  const role = method === 'getD1ChatSession'
    ? String(args && args[0] || '')
    : 'educator';
  const phone = method === 'getD1ChatSession'
    ? String(args && args[1] || '')
    : '';
