(function(){
  ...
  const original={
    chats:transport.chats.bind(transport),thread:transport.thread.bind(transport),sendMessage:transport.sendMessage.bind(transport),markRead:transport.markRead.bind(transport),markUnread:transport.markUnread.bind(transport),remove:transport.remove.bind(transport),pin:transport.pin.bind(transport)
  };
  ...
  transport.thread=function(session,phone,before,limit){const key=threadKey(session,phone,before,limit),cached=threadCache.get(key);if(cached){if(now()-cached.at>5000)fetchThread(session,phone,before,limit).catch(()=>{});return Promise.resolve(cached.value)}return fetchThread(session,phone,before,limit)};
  ...
  let startedForToken='';
  function concurrency(){const c=navigator.connection||navigator.mozConnection||navigator.webkitConnection;if(c&&c.saveData)return 1;const type=String(c&&c.effectiveType||'').toLowerCase();if(type==='slow-2g'||type==='2g')return 1;if(type==='3g')return 2;return /Android/i.test(navigator.userAgent||'')?3:4}
  async function warmThreads(session,chats){const seen=new Set(),queue=[];(chats||[]).forEach(chat=>{const p=phone10(chat&&chat.phone);if(!p||seen.has(p))return;seen.add(p);queue.push(p)});let cursor=0;const worker=async()=>{while(cursor<queue.length){const phone=queue[cursor++];if(!sameSession(savedSession(),session))return;try{await fetchThread(session,phone,'',100)}catch(_){}}};await Promise.all(Array.from({length:Math.min(concurrency(),queue.length)},worker))}
  async function start(session){if(!session||!session.token||startedForToken===session.token)return;startedForToken=session.token;try{const [u,r]=await Promise.all([fetchList(session,'unread').catch(()=>({chats:[]})),fetchList(session,'read').catch(()=>({chats:[]}))]);if(!sameSession(savedSession(),session))return;const unread=Array.isArray(u&&u.chats)?u.chats:[],read=Array.isArray(r&&r.chats)?r.chats:[];await warmThreads(session,unread.concat(read))}catch(_){}}
  ...
  kick();
  const timer=setInterval(()=>{const s=savedSession();if(s&&s.token!==startedForToken)start(s);else if(!s)ensureSession().catch(()=>{});if(startedForToken&&s&&s.token===startedForToken)clearInterval(timer)},180);
  setTimeout(()=>clearInterval(timer),30000);
  window.MedsiEducatorPrewarm={kick,clear(){listCache.clear();threadCache.clear();startedForToken=''}};
})();
