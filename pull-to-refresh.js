(function(){
  ...
  function mount(){if(!indicator.isConnected)document.body.appendChild(indicator)}
  function scrollTop(registration){
    const scroller=registration.getScroller&&registration.getScroller();
    if(!scroller||scroller===window||scroller===document||scroller===document.body||scroller===document.documentElement)return Math.max(window.scrollY||0,document.documentElement.scrollTop||0,document.body.scrollTop||0);
  ...
  function setPull(distance){
    const eased=Math.min(74,Math.max(0,distance)*.72);
    indicator.style.setProperty('--medsi-pull-offset',eased+'px');
    indicator.style.setProperty('--medsi-pull-opacity',String(Math.min(1,eased/34)));
    indicator.setAttribute('aria-hidden',eased>0?'false':'true');
    indicator.classList.toggle('is-ready',eased>=52);
  }
  function hide(){
    pulling=false;active=null;indicator.classList.remove('is-ready','is-refreshing');
  ...
  async function runRefresh(registration){
    ...
    indicator.classList.remove('is-ready');indicator.classList.add('is-refreshing');
    indicator.setAttribute('aria-label','Обновляем');
    indicator.setAttribute('aria-hidden','false');
    indicator.style.setProperty('--medsi-pull-offset','52px');indicator.style.setProperty('--medsi-pull-opacity','1');
  ...
  function touchMove(event){
    ...
    if(dy<7)return;
    pulling=true;if(event.cancelable)event.preventDefault();setPull(dy);
  }
  function touchEnd(){
